...
pub async fn inertial_measurement_read(i2c_bus: &'static I2c0BusShared) {
    ...
    sensor.set_digital_lowpass_filter(filter);

    sensor.set_accel_full_scale(AccelFullScale::G2).await;
    let accel_offsets = Accel::new(396, 324, 16616);
    sensor.set_accel_calibration(&accel_offsets);

    sensor.set_gyro_full_scale(GyroFullScale::Deg2000).await;
    let gyro_offsets = Gyro::new(3, 29, -8);
    sensor.set_gyro_calibration(&gyro_offsets);
