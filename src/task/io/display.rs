...
pub enum DisplayAction {
    /// Show radar sweep from corrected sweep buffer points.
    /// Draws the arc, sweep line at `current_angle`, and all obstacle points
    /// read from `ultrasonic::sweep_buffer_points()` at render time.
    ShowSweepFromBuffer {
        /// Current servo angle for the sweep line (degrees, 0-160).
        current_angle: f32,
    },
...
pub async fn display(i2c_bus: &'static I2cBusShared) {
    ...
    loop {
        ...
        if let Err(error) =
            handle_display_action(&mut display, text_style_bold, text_style_regular, display_action).await
        {
            defmt::warn!(
...
/// Pre-computed display coordinates for a radar sweep rendering.
struct SweepCoords {
    /// Sweep line endpoint.
    line_end: Point,
    /// Obstacle pixel positions within display bounds.
    pixels: HeaplessVec<Point, 32>,
}

/// Compute sweep line endpoint and obstacle pixel positions from sensor points.
///
/// Pure function — no async, no hardware. Converts polar coordinates
...
async fn handle_display_action(
    ...
    match display_action {
        DisplayAction::ShowSweepFromBuffer { current_angle } => {
            let points = ultrasonic::sweep_buffer_points().await;
            let coords = compute_sweep_coords(&points, current_angle);
            ...
            // Draw sweep line
            Line::new(Point::new(CENTER_X, CENTER_Y), coords.line_end)
                .into_styled(PrimitiveStyle::with_stroke(BinaryColor::On, 1))
                .draw(display)
                .map_err(|_| DisplayError::DrawError)?;

            // Draw all obstacle points
            for pixel in &coords.pixels {
                Pixel(*pixel, BinaryColor::On)
                    .draw(display)
                    .map_err(|_| DisplayError::DrawError)?;
            }
