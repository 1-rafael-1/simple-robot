...
pub async fn render_current_ui(state: &UiState) {
    match state.mode {
        ...
        UiMode::RunningCoastAvoidDetectionTest => {
            render_coast_avoid_detection_test_running().await;
        }
...
/// Render the coast-avoid detection test placeholder screen.
pub async fn render_coast_avoid_detection_test_running() {
    display::display_update(DisplayAction::Clear).await;
    show_line(0, "C-A Detect Test").await;
    show_line(1, "IR: ----").await;
    show_line(2, "US: ----").await;
    show_line(3, "=> ----").await;
}
